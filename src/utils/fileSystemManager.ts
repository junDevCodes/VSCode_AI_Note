...
export class FileSystemManager {
  ...
  static getWorkspaceRootUri(): vscode.Uri | undefined {
    ...
    NotificationManager.showWarning(
      "VS Code 작업 공간(폴더)이 열려있지 않습니다. 먼저 폴더를 여세요."
    );
    return undefined;
  }
  ...
  static async createDirectory(
    ...
    try {
      ...
      NotificationManager.showError(
        `디렉토리 생성 실패: ${path.basename(fsPath)} - ${error.message}`
      );
  ...
  static async writeFile(
    ...
    try {
      ...
      NotificationManager.showInformation(
        `파일 생성/업데이트: ${path.basename(fsPath)}`
      );
      return true;
    } catch (error: any) {
      NotificationManager.showError(
        `파일 생성/업데이트 실패: ${path.basename(fsPath)} - ${error.message}`
      );
  ...
  static async openFileInEditor(filePath: string | vscode.Uri): Promise<void> {
    ...
    try {
      ...
      NotificationManager.showError(
        `파일 열기 실패: ${path.basename(fileUri.fsPath)} - ${error.message}`
      );
